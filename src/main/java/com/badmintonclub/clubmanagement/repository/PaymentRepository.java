...
public interface PaymentRepository extends JpaRepository<Payment, Long> {
        ...
        @Query("""
                         SELECT p
                         FROM Payment p
                         WHERE p.batch.month = :month
                         ORDER BY p.batch.id DESC, p.user.fullName ASC
                        """)
        List<Payment> findPaymentsByMonth(@Param("month") String month);

        boolean existsByBatchAndUser(PaymentBatch batch, User user);
        ...
        @Query("""
                         ...
                         WHERE p.batch = :batch
                        """)
        List<Long> findUserIdsByBatch(@Param("batch") PaymentBatch batch);
}